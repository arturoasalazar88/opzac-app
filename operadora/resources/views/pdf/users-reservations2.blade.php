            ...
            <table>
                ...
                <tbody>
                    @if ($reservations->count() == 0)
                        ...
                        <tr class="alert alert-danger" colspan="20">
                            <td>Lo sentimos, no hay registro de pagos para la fecha indicada</td>
                        </tr>
                    @else
                        @php
                            $lastTour = $reservations->first()->tour_id;
                            $total = 0;
                        @endphp
                        @forelse ($reservations as $key => $reservation)

                            @if ($lastTour != $reservation->tour_id)
                                {{ $lastTour = $reservation->tour_id }}
                                <tr>
                                    {{-- <td>Aquí cerramos {{ $lastTour }}</td> --}}
                                    <td colspan="6">Total = {{ $total }}</td>
                                </tr>
                                <tr>
                                    <td scope="col">Folio</td>
                                    <td scope="col">Tour</td>
                                    <td scope="col">Horario</td>
                                    <td scope="col">Fecha</td>
                                    <td scope="col">M. Pago</td>
                            ...
                            <tr>
                                ...
                                {{-- <td>{{ $reservation->departure->tour->name }}

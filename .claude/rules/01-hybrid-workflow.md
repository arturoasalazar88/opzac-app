...
- Quality checks (`./vendor/bin/pint`, `./vendor/bin/phpstan`)
- Git operations
- System commands
...
gemini -m gemini-2.5-pro "@pattern.php @.context/06-coding-standards.md Generate [component] following Context7 patterns. Output code as text ONLY. DO NOT create files." 2>&1 | grep -v "^\[" | grep -v "^Loaded"
```
...
gemini -m gemini-2.5-pro --yolo "Run ./vendor/bin/phpunit tests/Unit/NewCodeTest.php"
```
...
### Example 1: Service Generation

**Claude → Gemini:**
```bash
gemini -m gemini-2.5-pro "@app/Services/TokenService.php @.context/06-coding-standards.md Generate ReservationCacheService with methods: getCachedReservation(tourId, date), cacheReservation(tourId, date, data, ttlHours=1), invalidateCache(tourId, date), getCacheStats(). Follow Laravel service patterns from TokenService.php. Output complete code as text ONLY."
```
...
/**
 * Handle reservation caching with automatic TTL expiration
 */
class ReservationCacheService
{
    /**
     * Retrieve cached reservation by tour and date
     *
     * @param int $tourId
     * @param string $date
     * @return array|null
     */
    public function getCachedReservation(int $tourId, string $date): ?array
    {
        $key = "reservation:{$tourId}:{$date}";
...
**Claude Reviews:**
- ✅ Follows Laravel service patterns
- ✅ PHPDoc complete
- ✅ Type hints present
- ⚠️  Missing cache driver config check (Claude adds separately)
...
**Claude → Gemini:**
```bash
gemini -m gemini-2.5-pro --allowed-mcp-server-names context7 "@app/Services/ReservationService.php Generate PHPUnit tests for ReservationService following Context7 best practices for Laravel testing. Include setUp, mocks, success/error cases. Output complete test code as text ONLY."
```

**Gemini → Claude:** [Complete test file with PHPUnit setup, mocks, test methods]
...
gemini -m gemini-2.5-pro "@pattern.php Generate code following Context7 patterns. Output as text."
```
...
gemini -m gemini-2.5-pro 'google_web_search(query="Laravel Cache Redis official documentation")' 2>&1 | grep -v "^\[ERROR\]" | grep -v "^Loaded"
# Output: Links to Laravel docs, key methods summary

# Step 2: Use Context7 for implementation patterns
gemini -m gemini-2.5-pro --allowed-mcp-server-names context7 "Search Context7 for Laravel Cache best practices. REPORT key patterns."
# Output: Code examples and patterns

# Step 3: Generate code with both insights
gemini -m gemini-2.5-pro "@pattern.php Generate CacheService with get, put methods following discovered patterns. Output as text." 2>&1 | grep -v "^\[" | grep -v "^Loaded"
```

**Example - Follow-up Instructions:**
```bash
# Search + immediate processing
gemini -m gemini-2.5-flash 'google_web_search(query="Laravel 5.7 installation requirements") Provide the composer command and link to docs.' 2>&1 | grep -v "^\[ERROR\]" | grep -v "^Loaded"
# Output: composer create-project laravel/laravel + URLs
```
...
**Measurement:**
```bash
# Calculate Gemini-generated lines (track in implementation log)
GEMINI_LINES=$(find app/ -name "*.php" -exec wc -l {} + | grep -i "gemini" | awk '{sum+=$1} END {print sum}')
TOTAL_LINES=$(find app/ -name "*.php" -exec wc -l {} + | tail -1 | awk '{print $1}')
